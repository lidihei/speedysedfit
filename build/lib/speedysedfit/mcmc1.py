...
def lnprior(theta, derived_properties, limits, **kwargs):
    ...
    constriants = setup.pop('constraints')
    ...
    if 'teff' in constraints:
       teff, teff_err = constraints['teff']
       teffprior = -0.5 * (pars[0] - teff)**2/teff_err**2
    else: teffprior = 0
    if 'logg' in constraints:
       logg, logg_err = constraints['logg']
       loggprior = -0.5 * (par[1] - logg)**2/logg_err**2
    else:loggprior = 0
    if 'teff2' in constraints:
       teff2, teff2_err = constraints['teff2']
       teff2prior = -0.5 * (par[3] - teff2)**2/teff2_err**2
    else:teff2prior=0
    if 'logg2' in constraints:
       logg2, logg2_err = constraints['logg2']
       logg2prior = -0.5 * (par[4] - logg2)**2/logg2_err**2
    else:logg2prior=0
